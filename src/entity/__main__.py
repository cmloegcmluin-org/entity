...
def _agent_protocol_note(roster, logs):
    ...
    return (
        ...
        "them. Write it whenever they said anything that deserves a reply, which is nearly always: "
        "they asked to be shown the work running and heard, in full, 'Passed that to <agent>.' "
        "Their question was answered and the answer thrown away. Say the sentence FIRST, then the "
        "marker on its own line. With no sentence they get a bare confirmation, which is fine only "
        "when a bare confirmation is genuinely the whole of what you have to say. "
