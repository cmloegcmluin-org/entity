...
def test_either_half_can_be_left_out():
    # He says one or the other as often as both, and the half he didn't mention is not a reason to
    # ignore the half he did.
    assert resolve("use Fable") == ("claude-fable-5", None)
    assert resolve("crank it to max") == (None, "max")


def test_a_full_model_id_is_his_choice_too():
...
def test_a_phrase_with_no_choice_in_it_resolves_to_nothing():
    # So the caller can say it didn't understand, rather than quietly running his work on whatever
    # a guess landed on.
